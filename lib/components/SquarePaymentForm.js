...
var SquarePaymentForm = (function (_super) {
    ...
    function SquarePaymentForm(props) {
        ...
        _this.cardNonceResponseReceived = function (errors, nonce, cardData, billingContact, shippingContact) {
            if (errors || !_this.props.createVerificationDetails) {
                _this.props.cardNonceResponseReceived(errors, nonce, cardData, billingContact, shippingContact);
                return;
            }
            _this.paymentForm && _this.paymentForm.verifyBuyer(nonce, _this.props.createVerificationDetails(billingContact, shippingContact), function (err, result) {
                if (err) {
                    console.log('error from verification details check: ', err);
                    return _this.props.cardNonceResponseReceived(err, nonce, cardData, billingContact, shippingContact, '');
                }
                console.log('result from verification details check: ', result);
                _this.props.cardNonceResponseReceived(err, nonce, cardData, billingContact, shippingContact, result.token);
            });
        };
