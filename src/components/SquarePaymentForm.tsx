...
class SquarePaymentForm extends React.Component<SquarePaymentFormProps, State> {
  ...
  cardNonceResponseReceived = (errors: [SqError], nonce: string, cardData: SqCardData, billingContact: SqContact, shippingContact: SqContact) => {
    ...
    this.paymentForm && this.paymentForm.verifyBuyer(
      ...
      (err: [SqError], result: SqVerificationResult) => {
        if (err) {
          ...
          this.props.cardNonceResponseReceived(err, nonce, cardData, billingContact, shippingContact, '')
          return
        }
